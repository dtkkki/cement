...
// ParsePushEvent event from a github push event payload.
func ParsePushEvent(payload []byte) (hooks.PushEvent, error) {
	return DefaultEventParser.ParsePushEvent(payload)
}

// EventParser is a payload parser for github.
type EventParser struct {
}

// ParsePushEvent parse event from a github push event payload.
func (parser *EventParser) ParsePushEvent(payload []byte) (hooks.PushEvent, error) {
	...
	repo := hooks.Repo{
		...
		HomePage:    gjson.Get(s, "repository.html_url").String(),
	...
	gjson.Get(s, "commits").ForEach(func(_, value gjson.Result) bool {
		author := hooks.Author{
			Name:  value.Get("committer").Get("name").String(),
			Email: value.Get("committer").Get("email").String(),
		}
		commit := hooks.Commit{
